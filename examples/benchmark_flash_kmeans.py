...
from flash_kmeans import batch_kmeans_Cosine, batch_kmeans_Euclid
...
def _euclid_iter_torch(x, x_sq, centroids):
    ...
    dist_sq = (
        x_sq[:, :, None] + cent_sq[:, None, :] - 2.0 * cross
    ).clamp_min_(0.0)
...
def batch_kmeans_Euclid_torch(
    ...
    for it in range(max_iters):
        ...
        centroids_new, center_shift, cluster_ids = _euclid_iter_torch(
            x, x_sq, centroids
        )
...
def batch_kmeans_Cosine_torch(
    ...
    for it in range(max_iters):
        # ---- compiled single iteration ----
        centroids_new, center_shift, cluster_ids = _cosine_iter_torch(
            x, centroids
        )
...
try:
    ...
    def batch_kmeans_Euclid_fast_torch(
        x,
        n_clusters,
        max_iters=100,
        tol=0.0,
    ...
    def batch_kmeans_Cosine_fast_torch(
        x,
        n_clusters,
        max_iters=100,
        tol=0.0,
...
try:
    ...
    def batch_kmeans_Euclid_fastkmeans(
        x,
        n_clusters,
        max_iters=100,
        tol=0.0,
    ...
    def batch_kmeans_Euclid_fastkmeans_torch(
        x,
        n_clusters,
        max_iters=100,
        tol=0.0,
...
def benchmark_kmeans_all(
    ...
    with open(output_file, "a") as output:
        for kmeans_func in kmeans_func_list:
            ...
            try:
                t = benchmark_kmeans(
                    b, n, d, k, kmeans_func, max_iters=max_iters, tol=tol
                )
                print(
                    f"Time for {b}x{n}x{d}x{k} with {max_iters} iterations: {t:.2f} ms"
                )
                ...
                output.write(
                    f'{{"method": "{kmeans_func.__name__}", "batch_size": {b}, "num_points": {n}, "dim": {d}, "num_clusters": {k}, "max_iters": {max_iters}, "tol": {tol}, "time_ms": {t}, "time_per_iter_ms": {t / max_iters}}}\n'
                )
                output.flush()
            print()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="use_heuristic",
        action="store_true",
        help="Use heuristic Triton config for flash-kmeans (Euclid only)",
    )
