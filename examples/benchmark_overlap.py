...
def kmeans_largeN_no_overlap(
    ...
    blocks_per_gpu = [
        (num_blocks // G) + (1 if g < num_blocks % G else 0) for g in range(G)
    ]
    block_start_g = [0] * G
    for g in range(1, G):
    ...
    for g in active_gpus:
        ...
        with torch.cuda.device(dev):
            ...
            cluster_ids_g[g] = torch.empty(
                (n_pts,), device=dev, dtype=torch.int32
            )
    ...
    for it in range(max_iters):
                ...
                with torch.cuda.stream(streams[g]):
                    for local_idx in range(blocks_per_gpu[g]):
                        ...
                        x_block = x[n_start:n_end].to(
                            dev, non_blocking=False, dtype=dtype
                        )
        ...
        with torch.cuda.device(primary_dev):
            ...
            shift = (
                (new_centroids - centroids_g[primary_gpu]).norm(dim=-1).max()
            )
...
def main():
    ...
    if num_gpus == 1:
        ...
        _mod._resolve_devices = lambda dev: [
    ...
    print(
        f"  Non-overlapped:  {ms_no:10.1f} ms total,  {iter_no:8.2f} ms/iter"
    )
    ...
    print(
        f"  Overlapped:      {ms_ov:10.1f} ms total,  {iter_ov:8.2f} ms/iter"
    )
    ...
    print("-" * 55)
    print(
        f"{'Non-overlapped':<20} {ms_no:>12.1f} {iter_no:>10.2f} {'1.00x':>10}"
    )
    speedup = ms_no / ms_ov
    print(
        f"{'Overlapped':<20} {ms_ov:>12.1f} {iter_ov:>10.2f} {speedup:>9.2f}x"
    )
    print("=" * 55)
