...
def euclid_assign_torch_native_chunked(
    x, centroids, x_sq, chunk_size_N=32768, chunk_size_K=1024
):
    """
    ...
    K = centroids.shape[1]
    assert (
        centroids.shape[2] == x.shape[2]
    ), "Dimension mismatch between x and centroids"
    assert (
        x.shape[0] == centroids.shape[0]
    ), "Batch size mismatch between x and centroids"
    ...
    for n_start in range(0, N, chunk_size_N):
        ...
        for k_start in range(0, K, chunk_size_K):
            ...
            dists_partial = (
                ...
                - 2
                * torch.bmm(
                    x_chunk, cent_chunk.transpose(1, 2)
                )  # (B, n_chunk, k_chunk)
...
def torch_loop_centroid_update(
    x_norm: torch.Tensor,
    ...
    for b in range(B):
        for k in range(K):
            ...
            if mask.any():
                if mode == "euclid":
                    new_centroids[b, k] = x_norm[b][mask].mean(
                        dim=0, dtype=x_norm.dtype
                    )
                else:  # cosine
                    new_centroids[b, k] = F.normalize(
                        x_norm[b][mask].mean(dim=0, dtype=x_norm.dtype),
                        p=2,
                        dim=0,
                    )
            else:
...
def _centroid_update_torch_native(
    x, cluster_ids, old_centroids, mode="euclid"
):
    """
...
def _euclid_iter_torch_naive(
    x, x_sq, centroids, chunk_size_N=32768, chunk_size_K=1024
):
    """
    ...
    cluster_ids = euclid_assign_torch_native_chunked(
        x, centroids, x_sq, chunk_size_N, chunk_size_K
    )
...
def batch_kmeans_Euclid_torch_native(
    ...
    for it in range(max_iters):
        centroids_new, center_shift, cluster_ids = _euclid_iter_torch_naive(
            x, x_sq, centroids, chunk_size_N, chunk_size_K
        )
