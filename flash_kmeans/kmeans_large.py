...
def kmeans_largeN(
    ...
    blocks_per_gpu = [
        (num_blocks // G) + (1 if g < (num_blocks % G) else 0)
        for g in range(G)
    ]
    block_start = [0] * G
    for g in range(1, G):
    ...
    point_end = [
        min(point_start[g] + blocks_per_gpu[g] * BLOCK_N, N) for g in range(G)
    ]
    ...
    for g in active_gpus:
        ...
        with torch.cuda.device(dev):
            work_streams[g] = [
                torch.cuda.Stream(device=dev) for _ in range(buf_size)
            ]
            reduce_stream[g] = torch.cuda.Stream(device=dev)
            n_points_g = point_end[g] - point_start[g]
            cluster_ids_g[g] = torch.empty(
                (n_points_g,), device=dev, dtype=torch.int32
            )
    ...
    with torch.cuda.device(primary_dev):
        with torch.cuda.stream(reduce_stream[primary_gpu]):
            if init_centroids is not None:
                centroids = init_centroids.to(
                    device=primary_dev, dtype=dtype, non_blocking=True
                )
            else:
                indices = torch.randint(
                    0, N, (K,), device="cpu", dtype=torch.int32
                )
                centroids = x[indices].to(
                    device=primary_dev, dtype=dtype, non_blocking=True
                )
        ...
        with torch.cuda.device(dev):
            with torch.cuda.stream(reduce_stream[g]):
                if g == primary_gpu:
                    ...
                    centroids_g[g] = torch.empty(
                        (K, D), device=dev, dtype=centroids.dtype
                    )
    ...
    if G > 1:
        with torch.cuda.device(primary_dev):
            staging_sums = {
                g: torch.empty((K, D), device=primary_dev, dtype=torch.float32)
                for g in active_gpus
                if g != primary_gpu
            }
            staging_cnts = {
                g: torch.empty((K,), device=primary_dev, dtype=torch.int32)
                for g in active_gpus
                if g != primary_gpu
            }
    ...
    for g in active_gpus:
        ...
        with torch.cuda.device(dev):
            done_events[g] = [
                torch.cuda.Event() for _ in range(blocks_per_gpu[g])
            ]
    ...
    for it in range(max_iters):
        ...
        for g in active_gpus:
            ...
            with torch.cuda.device(dev):
                with torch.cuda.stream(reduce_stream[g]):
                    centroid_sums_g[g] = torch.zeros(
                        (K, D), device=dev, dtype=torch.float32
                    )
                    centroid_cnts_g[g] = torch.zeros(
                        (K,), device=dev, dtype=torch.int32
                    )
            ...
            with torch.cuda.device(dev):
                for local_idx in range(local_blocks):
                    ...
                    with torch.cuda.stream(ws):
                        ...
                        x_block = x[n_start:n_end].to(
                            dev, non_blocking=True, dtype=dtype
                        )

                        # Cache x_sq on first iteration
                        if x_sq_cache_g[g][local_idx] is None:
                            x_sq_cache_g[g][local_idx] = (x_block**2).sum(
                                dim=-1
                            )
                        ...
                        local_end = min(
                            local_offset + BLOCK_N, point_end[g] - ps
                        )
        ...
        with torch.cuda.device(primary_dev):
            with torch.cuda.stream(reduce_stream[primary_gpu]):
                ...
                for g in active_gpus:
                    reduce_stream[primary_gpu].wait_event(
                        done_events[g][blocks_per_gpu[g] - 1]
                    )
                ...
                if G == 1:
                    ...
                    for g in active_gpus:
                        if g != primary_gpu:
                            staging_sums[g].copy_(
                                centroid_sums_g[g], non_blocking=True
                            )
                            staging_cnts[g].copy_(
                                centroid_cnts_g[g], non_blocking=True
                            )
                    ...
                    new_centroids = torch.where(
                        mask.unsqueeze(-1),
                        total_sums
                        / total_cnts.clamp(min=1).unsqueeze(-1).float(),
                        centroids_g[primary_gpu].float(),
                    ).to(dtype)

                shift = (
                    (new_centroids - centroids_g[primary_gpu])
                    .norm(dim=-1)
                    .max()
                )
                ...
                if verbose:
                    print(
                        f"Iter {it}, center shift: {shift.item():.6f}, time: {time.time() - start_time:.2f}s"
                    )
                ...
                if shift < tol:
                    ...
                    if G > 1:
                        for g in active_gpus:
                            ps_g = point_start[g]
                            pe_g = point_end[g]
                            cluster_ids_cpu[ps_g:pe_g].copy_(
                                cluster_ids_g[g][: pe_g - ps_g]
                            )
                    break
                ...
                for g in active_gpus:
                    if g != primary_gpu:
                        with torch.cuda.device(devices[g]):
                            centroids_g[g].copy_(
                                new_centroids, non_blocking=True
                            )
...
def kmeans_largeN_assign(
    ...
    assert (
        centroids.shape[1] == D
    ), "centroids and x must have the same feature dimension"
    ...
    for g in active_gpus:
        ...
        with torch.cuda.device(dev):
            ...
            if g == primary_gpu:
                ...
                centroids_g[g] = centroids_on_primary.to(
            ...
            for local_idx in range(local_blocks):
                ...
                with torch.cuda.stream(ws):
                    ...
                    euclid_assign_triton(
                        ...
                        cent_g.unsqueeze(0),
                        x_sq_cache_g[g][local_idx].unsqueeze(0),
                        out=cluster_ids_g[g][local_offset:local_end].unsqueeze(
                            0
                        ),
...
if __name__ == "__main__":
    ...
    print(
        f"Time taken: {(end_time-start_time)*1e3:.2f}ms  {(end_time-start_time)*1e3/10:2f} ms/pre"
    )
